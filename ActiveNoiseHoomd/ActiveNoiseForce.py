...
class ActiveNoiseForce(hoomd.md.force.Custom):
    ...
    def do_interpolation_test(self):
        ...
        if self._dim==2:
            ...
            thenoise_x1 = self._field[0,-1,0,0,0]*0.5 + self._field[0,0,0,0,0]*0.5
            thenoise_x2 = self._field[0,-1,-1,0,0]*0.5 + self._field[0,0,-1,0,0]*0.5
            thenoise_x = thenoise_x1*0.5 + thenoise_x2*0.5

            thenoise_y1 = self._field[1,-1,0,0,0]*0.5 + self._field[1,0,0,0,0]*0.5
            thenoise_y2 = self._field[1,-1,-1,0,0]*0.5 + self._field[1,0,-1,0,0]*0.5
            thenoise_y = thenoise_y1*0.5 + thenoise_y2*0.5

            thenoise_z1 = self._field[2,-1,0,0,0]*0.5 + self._field[2,0,0,0,0]*0.5
            thenoise_z2 = self._field[2,-1,-1,0,0]*0.5 + self._field[2,0,-1,0,0]*0.5
            thenoise_z = thenoise_z1*0.5 + thenoise_z2*0.5

        if self._dim==3:
            return
            if xp.abs(noise_force[0,0]-thenoise_x)>1e-10 or xp.abs(noise_force[0,1]-thenoise_y)>1e-10 or xp.abs(noise_force[0,2]-thenoise_z)>1e-10:
                print('ERROR: active noise interpolation not working')
                print('from function:', noise_force)
                print('by hand:', thenoise_x, thenoise_y, thenoise_z)
        else:
